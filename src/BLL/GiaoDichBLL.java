...
public class GiaoDichBLL {
    ...
    //Chuyển thiết bị
    public int KtTBPhong(String MaTB,String MaPhong) //Kiểm tra xem phòng đó đã có thiết bị chưa
    {
        int so=0;
        try {
            String sql = "select * from TBPhong where MaTB='"+MaTB+"' and MaPhong='"+MaPhong+"'";
            ResultSet rs = dal.getResult(sql);
            if(rs.isBeforeFirst())
                so=1;
        } catch (SQLException e) {
